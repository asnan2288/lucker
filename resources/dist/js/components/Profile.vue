<template>
    <div class="users-box ml-auto cards" @click="$bvModal.show('settings')">
        <div class="pl-2" style="text-align: right;">
            <div class="users-login">{{ trimmedUsername }}</div>
    ...
    <path id="XMLID_105_" style="fill:#F0C41B;" d="M37.784,13.359c0.82-0.174,1.301-0.68,1.124-1.257l-0.19-0.616
		...
		C36.547,16.132,37.55,14.835,37.784,13.359"/>
                ...
                <IOdometer
                    ...
                    v-if="$root.animate.odometer || !$root.animate.odometer && !$root.animate.countup"
                />
                <ICountUp
                    :endVal="$root.user.balance"
                    :options="options"
                    v-if="$root.animate.countup"
                />
            </div>
        </div>
        <div class="users-photo flex ml-2">
            <img
                :src="$root.user.avatar"
                height="40"
                style="width: auto;height: 55px;border-radius: 10px;"
            />
        </div>
        <Settings />
        <Promocode />
...
import ConnectTG from "./modals/ConnectTG";
...
export default {
    ...
    computed: {
        trimmedUsername() {
            return this.$root.user.username.split(' ')[0];
        },
    },
    data() {
        return {
